...
class _AssessmentPageState extends State<AssessmentPage> {
  ...
  Future<double> fetchGroundwaterLevel(String district) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("Groundwater API response: $double.tryParse(data[0]['groundwaterlevel'].toString())"); // log API data
        if (data.isNotEmpty) {
  ...
  Future<Map<String, dynamic>?> fetchEnvironmentData(String state, String district) async {
  try {
    final url = Uri.parse("http://127.0.0.1:8000/environment/$state/$district");
    final response = await http.get(url);

    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      debugPrint("Environment API response: $data"); // logs entire JSON
      return data;
    } else {
      debugPrint("Environment API returned status: ${response.statusCode}");
    }
  } catch (e) {
...
void getEnvData() async {
  final data = await fetchEnvironmentData("maharashtra", "solapur");
  
  if (data != null) {
    print("Latest Groundwater Level: ${data['groundwater']['groundwater_level_m']}");
    print("Aquifer Type: ${data['groundwater']['aquifer_type']}");
    print("Latest Soil Moisture: ${data['soil_moisture']['soil_moisture']}");
  }
}
  ...
  debugPrint("Rainfall API response: $rainfallData");
  ...
  debugPrint("Aquifer API response: $aquiferData");
  ...
  if (selectedAssessment == "AR") {
    ...
    if (potentialLiters < annualDemand) {
      structure = "Small tank on rooftop";
    } else if (potentialLiters <= 2.5 * annualDemand) {
  ...
  Map<String, List<int>> costRanges = {
    "Small tank on rooftop": [15000, 25000],
    "Medium-sized surface tank": [40000, 60000],
    "Large underground tank": [80000, 120000],
    "Recharge Shaft": [30000, 50000],
    "Recharge Pit": [50000, 80000],
    "Percolation Pond": [150000, 250000],
  };

  List<int>? range = costRanges[structure];
  double cost = range != null ? ((range[0] + range[1]) / 2) : 0;
  ...
  Future<Map<String, dynamic>> _fetchRainfallData(String location) async {
    try {
      ...
      debugPrint("Geocoding API response: $geoData");
      ...
      debugPrint("NASA Rainfall API response: $nasaData");
  ...
  Future<Map<String, dynamic>?> fetchAquiferData(String district) async {
    ...
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      debugPrint("Aquifer API response: $data");
                        ...
                        children: [
                          ...
                          if (selectedAssessment == "Rooftop") ...[
                            customTextField(
                              ...
                              icon: Icons.roofing,
                              validator: (value) {
                                if (value == null || value.isEmpty) return "Enter rooftop area";
                            ...
                            customTextField(
                              ...
                              validator: (value) {
                                if (value == null || value.isEmpty) return "Enter open space area";
                                if (double.tryParse(value) == null) return "Enter valid number";
                                return null;
                              },
                            ),
                            ...
                            DropdownButtonFormField<String>(
                              value: _roofType,
                              items: ["concrete", "gi_sheet", "asbestos"].map((type) {
                            ...
                            customTextField(
                              ...
                              icon: Icons.people,
                              validator: (value) {
                                if (value == null || value.isEmpty) return "Enter number of dwellers";
                            ...
                            customTextField(
                              ...
                              icon: Icons.height,
                              validator: (value) {
                                if (value == null || value.isEmpty) return "Enter well depth";
                                if (double.tryParse(value) == null) return "Enter valid number";
                                return null;
                              },
                            ),
                            customTextField(
                              ...
                              icon: Icons.circle,
                              validator: (value) {
                                if (value == null || value.isEmpty) return "Enter well diameter";
                                if (double.tryParse(value) == null) return "Enter valid number";
                                return null;
                              },
                            ),
